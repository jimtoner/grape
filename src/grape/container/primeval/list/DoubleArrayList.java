...
public class DoubleArrayList implements RandomAccess, Cloneable, Serializable {
    ...
    public DoubleArrayList(double[] values, int value_begin, int len) {
	...
	public void add(int index, DoubleArrayList c) {
		if (index < 0 || index > size)
			throw new IndexOutOfBoundsException("Index:" + index + " size:"
					+ size);
	...
	public DoubleArrayList subList(int begin, int len) {
		if (begin < 0 || len < 0 || begin + len > size)
			throw new IllegalArgumentException();

		return new DoubleArrayList(buffer, begin, len);
	}
