...
public class CharObjectHashMap <V> implements Cloneable, Serializable {
    ...
    public CharObjectHashMap(int capacity) {
    ...
    public void putAll(CharObjectHashMap<? extends V> m) {
        if (m.size == 0)
            return;
    ...
    public Iterator<Entry<V> > iterator() {
    	return new Iterator<Entry<V>>() {
    		...
    		{
    			Entry<V>[] t = CharObjectHashMap.this.table;
    			for (int i = 0, len = t.length; i < len; ++i) {
    				if (t[i] != null) {
    					nextIndex = i;
    					nextEntry = t[i];
			...
			@Override
			public void remove() {
				CharObjectHashMap.this.remove(currentEntry.key);
			}
		};
    }
    ...
    private static int hash(char v) {
        // hash
        int h = v;
		// secondary hash
        h ^= (h >>> 20) ^ (h >>> 12);
	...
	public String toString() {
    	...
    	while (iter.hasNext()) {
    		...
    		if (sb.length() != 1)
    			sb.append(", ");
    		sb.append(Character.toString(e.key));
    		sb.append(":");
    		sb.append(e.value);
    	}
    	sb.append("}");
